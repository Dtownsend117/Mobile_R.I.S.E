...
1.) I build this years ago and only made minor adjustments to upload to github
2.) There are issues that still need worked out
3.) There is no microphone support, just text chat

\/ STILL NEED FIXED \/
Since I had moved on from this project to focus on other work, the following issues are still in the program:
- the news command either dims the screen or opens a breif cmd window, requires a Rise restart
- The health report command requires an elevated CMD window to display


FOR STARTING:
-In order to ge the best use of R.I.S.E, you will first need to go to the scripts folder located in the Mobile R.I.S.E folder
-Locate the "enableWSH" Registry file, double click to install it and then click OK on the windows tha appear after it
...
FOR TYPING:
-each command will need to be typed in all lowercase in order to work
...
-Some maybe all features might not be available depending on the administrator of the system, Those features include: Weather, News, Speech
